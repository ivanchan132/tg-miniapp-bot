...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=chat_id, text="Привет. Даже не знаю как так получился этот бот, но вот ты здесь. Так как ты просила не звонить, а просто написать мне показалось слишком банальным, получилась вот такая вот штука🫣.Она включает в себя небольшую игру, а что бы было интереснее играть при проигрыше с определнным счетом тебя будут ждать небольшие поздравления внутри игры (их всего четыре, от 5 до 35), само поздравление и подарок🎁, посленяя вкладка так и незаработала у меня, поэтому на нее внимание не обращай😂", reply_markup=kb)
